...
CWmcaIntf::CWmcaIntf()	:
	...
	m_pConnect(NULL), m_pConnectCert(NULL), m_pDisconnect(NULL),	
	m_pTransact(NULL),	m_pQuery(NULL),	m_pRequest(NULL),	m_pAttach(NULL),
...
{
	m_hDll = LoadLibrary("wmca.dll");

	if(!m_hDll)
	{
		AfxMessageBox("wmca.dll 파일을 로드할 수 없습니다.\n점검 후 재시도하시기 바랍니다.");
	...
	m_pConnectCert			=(TConnectCert*)			GetProcAddress(m_hDll,"wmcaConnectCert");
	...
	if(!(m_pLoad && m_pFree && m_pSetServer && m_pSetPort && m_pIsConnected && m_pConnect && m_pConnectCert && m_pDisconnect 
		&& m_pTransact && m_pQuery && m_pRequest && m_pAttach && m_pDetach && m_pDetachWindow && m_pDetachAll && m_pSetOption 
		&& m_pSetAccountIndexPwd && m_pSetOrderPwd && m_pSetHashPwd && m_pSetAccountNoPwd) && m_pSetAccountNoByIndex)
	{
		AfxMessageBox("wmca.dll 파일 점검 후 재시도하시기 바랍니다.");
...
BOOL	CWmcaIntf::ConnectCert(HWND hWnd, DWORD msg, char MediaType, char UserType, int mode)
{
	if (m_pConnectCert)
		return	m_pConnectCert(hWnd, msg, MediaType, UserType, mode);
