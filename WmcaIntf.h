...
typedef	BOOL	(__stdcall TConnectCert			)(HWND hWnd, DWORD dwMsg, char cMediaType, char cUserType, int mode);
...
public:
	BOOL Load				();
	BOOL Free				();
	BOOL Connect			(HWND hWnd, DWORD msg, char MediaType,char UserType,const char* szID,const char* szPW, const char* szCertPW);				//ID 로그인
	BOOL ConnectCert		(HWND hWnd, DWORD msg, char MediaType, char UserType, int mode);															//인증서 로그인
	BOOL Disconnect			();
