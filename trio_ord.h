...
typedef struct tagc8101InBlock    //기본입력
{
	...
	char issue_codez6[6];	char _issue_codez6;                       //종목번호             //필히6자리여야함
	...
	char shsll_pos_flagz1[1];	char _shsll_pos_flagz1;                   //공매도가능여부       /*0:정상(00)1:기타공매도(02)2:차입주식매도(01)3:~(99)4:기타공매도(02)이외:정상(00)*/
	char trad_pswd_no_1z8[44];	char _trad_pswd_no_1z8;                   //거래비밀번호1        
	char trad_pswd_no_2z8[44];	char _trad_pswd_no_2z8;                   //거래비밀번호2        
	char sop_cnd_prz15[15];	char _sop_cnd_prz15;                      //정지조건가격         /*매매유형코드가16:스톱지정가경우만세팅*/
	char rmt_mkt_cdz3[3];	char _rmt_mkt_cdz3;                       //요청시장코드         /*SOR:SOR,KRX:KRX,NXT:NXT,_:해당사항없음(코스피/코스닥/코넥스가아닌종목주문)*/
	char sor_mkt_sli_ynz1[1];	char _sor_mkt_sli_ynz1;                   //SOR시장분할여부      /*Y:스플릿허용,N:스플릿불가,_:해당사항없음(코스피/코스닥/코넥스가아닌종목주문)*/
	char sor_rule_cdz4[4];	char _sor_rule_cdz4;                      //SOR룰코드            /*A:A,B:B,_:미사용*/
	char mkt_insd_cor_req_ynz1[1];	char _mkt_insd_cor_req_ynz1;              //시장내정정신청여부   /*시장내정정신청여부고객:N,HIT,FIX:Y*/
} Tc8101InBlock;
...
typedef struct tagc8101OutBlock    //화면출력
{
	...
	char order_qtyz12[12];	char _order_qtyz12;                       //주문수량             
	char order_unit_pricez10[10];	char _order_unit_pricez10;                //주문단가             
	char sor_fle_idz20[20];	char _sor_fle_idz20;                      //SOR파일ID            
	char sor_ant_rt1z11[11];	char _sor_ant_rt1z11;                     //KRX배분비율          
	char sor_ant_rt2z11[11];	char _sor_ant_rt2z11;                     //NXT배분비율          
	char orr_qty1z18[18];	char _orr_qty1z18;                        //KRX주문수량          /*KRX주문수량*/
	char orr_qty2z18[18];	char _orr_qty2z18;                        //NXT주문수량          /*NXT주문수량*/
	char anw_cld_mkt_orr_no1z10[10];	char _anw_cld_mkt_orr_no1z10;             //KRX신규자시장주문번호 /*KRX시장주문번호*/
	char anw_cld_mkt_orr_no2z10[10];	char _anw_cld_mkt_orr_no2z10;             //NXT신규자시장주문번호 /*NXT시장주문번호*/
} Tc8101OutBlock;
...
typedef struct tagc8103InBlock    //기본입력
{
	...
	char all_part_typez1[1];	char _all_part_typez1;                    //정정구분             /*1:잔량(전체)2:일부(수량)*/
	char trad_pswd_no_1z8[44];	char _trad_pswd_no_1z8;                   //거래비밀1            
	char trad_pswd_no_2z8[44];	char _trad_pswd_no_2z8;                   //거래비밀2            
	char sop_cnd_prz15[15];	char _sop_cnd_prz15;                      //정지조건가격         /*매매유형코드가16:스톱지정가경우만세팅*/
	char rmt_mkt_cdz3[3];	char _rmt_mkt_cdz3;                       //요청시장코드         /*_:원주문과동일하게처리(시장분할,룰코드도원주문과동일)*/
	char sor_mkt_sli_ynz1[1];	char _sor_mkt_sli_ynz1;                   //SOR시장분할여부      /*_:원주문과동일*/
	char sor_rule_cdz4[4];	char _sor_rule_cdz4;                      //SOR룰코드            /*_:원주문과동일A:A,B:B,_:미사용*/
} Tc8103InBlock;
...
typedef struct tagc8103OutBlock    //화면출력
{
	...
	char crctn_pricez10[10];	char _crctn_pricez10;                     //정정단가             
	char sor_fle_idz20[20];	char _sor_fle_idz20;                      //SOR파일ID            
	char can_sor_ant_rt1z11[11];	char _can_sor_ant_rt1z11;                 //취소KRX배분비율      /*KRX*/
	char can_sor_ant_rt2z11[11];	char _can_sor_ant_rt2z11;                 //취소NXT배분비율      /*NXT*/
	char can_orr_qty1z18[18];	char _can_orr_qty1z18;                    //KRX취소주문수량      /*KRX*/
	char can_orr_qty2z18[18];	char _can_orr_qty2z18;                    //NXT취소주문수량      /*NXT*/
	char can_cld_mkt_orr_no1z10[10];	char _can_cld_mkt_orr_no1z10;             //KRX취소자시장주문번호 /*KRX*/
	char can_cld_mkt_orr_no2z10[10];	char _can_cld_mkt_orr_no2z10;             //NXT취소자시장주문번호 /*NXT*/
} Tc8103OutBlock;
...
typedef struct tagc8104InBlock    //기본입력
{
	...
	char orgnl_order_noz10[10];	char _orgnl_order_noz10;                  //원주문번호           
	char all_part_typez1[1];	char _all_part_typez1;                    //취소구분             /*1:잔량(전체)2:일부(수량)*/
	char trad_pswd_no_1z8[44];	char _trad_pswd_no_1z8;                   //거래비밀번호1        
	char trad_pswd_no_2z8[44];	char _trad_pswd_no_2z8;                   //거래비밀번호2        
} Tc8104InBlock;
...
typedef struct tagc8104OutBlock    //화면출력
{
	...
	char canc_qtyz12[12];	char _canc_qtyz12;                        //취소수량             
	char sor_fle_idz20[20];	char _sor_fle_idz20;                      //SOR파일ID            
	char can_sor_ant_rt1z11[11];	char _can_sor_ant_rt1z11;                 //취소SOR배분비율KRX   /*KRX*/
	char can_sor_ant_rt2z11[11];	char _can_sor_ant_rt2z11;                 //취소SOR배분비율NXT   /*NXT*/
	char can_orr_qty1z18[18];	char _can_orr_qty1z18;                    //취소주문수량KRX      /*KRX*/
	char can_orr_qty2z18[18];	char _can_orr_qty2z18;                    //취소주문수량NXT      /*NXT*/
	char can_cld_mkt_orr_no1z10[10];	char _can_cld_mkt_orr_no1z10;             //취소자시장주문번호KRX /*KRX*/
	char can_cld_mkt_orr_no2z10[10];	char _can_cld_mkt_orr_no2z10;             //취소자시장주문번호NXT /*NXT*/
} Tc8104OutBlock;
...
typedef struct tagc8201InBlock    //기본입력
{
	//char accnt_noz11[11];	char _accnt_noz11;                        //계좌번호             
	char pswd_noz8[44];	char _pswd_noz8;                          //비밀번호             
	char bnc_bse_cdz1[1];	char _bnc_bse_cdz1;                       //잔고구분             /*1:체결잔고,2:결제잔고,3:시간외종가체결잔고,4:시간외종가결제잔고*/
	char aet_bsez1[1];	char _aet_bsez1;                          //자산기준             /*1:순자산,2:총자산*/
	char qut_dit_cdz3[3];	char _qut_dit_cdz3;                       //시세구분코드         /*UNT(통합시세),KRX(KRX시세),NXT(NXT시세)*/
} Tc8201InBlock;
...
typedef struct tagc8201OutBlock    //화면출력
{
	...
	char nas_tot_amtz18[18];	char _nas_tot_amtz18;                     //순총자산금액         
	char nas_tot_txtz8[8];	char _nas_tot_txtz8;                      //순총자산타이틀       
} Tc8201OutBlock;
...
typedef struct tags8180InBlock    //기본입력
{
	//char accnt_noz11                      [ 11];	char _accnt_noz11;                        //계좌번호             
	char inq_gubunz1                      [  1];	char _inq_gubunz1;                        //조회주체구분         /*1:서비스계좌,2:그룹별,3:계좌별4.처리사원별조회*/
	char pswd_noz8                        [ 44];	char _pswd_noz8;                          //비밀번호             
	char group_noz4                       [  4];	char _group_noz4;                         //그룹번호             
	char mkt_slctz1                       [  1];	char _mkt_slctz1;                         //시장구분             /*0:전체1:삼일주문2:장내채권3:제3시장4:선물옵션5:장외단주6:ECN7:주식옵션현물*/
	char order_datez8                     [  8];	char _order_datez8;                       //주문일자             
	char issue_codez12                    [ 12];	char _issue_codez12;                      //종목번호             
	char comm_order_typez2                [  2];	char _comm_order_typez2;                  //매체구분             /*CC:전체AA:영업BB:온라인*/
	char conc_gubunz1                     [  1];	char _conc_gubunz1;                       //체결구분             /*0:전체1:미체결2:체결*/
	char inq_seq_gubunz1                  [  1];	char _inq_seq_gubunz1;                    //조회순서             /*0:번호1:모주문번호*/
	char sort_gubunz1                     [  1];	char _sort_gubunz1;                       //정렬구분             /*0:주문번호순1:주문번호역순*/
	char sell_buy_typez1                  [  1];	char _sell_buy_typez1;                    //매수도구분           /*0:전체1:매도2:매수3:전매4:환매*/
	char mrgn_typez1                      [  1];	char _mrgn_typez1;                        //신용구분             /*0:보통1:신용*/
	char accnt_admin_typez1               [  1];	char _accnt_admin_typez1;                 //계좌구분             /*0:전체1:법인12:국제3:선물옵션영업4:법인2*/
	char order_noz10                      [ 10];	char _order_noz10;                        //주문번호             
...
typedef struct tags8180OutBlock    //화면출력
{
...
typedef struct tags8180OutBlock1    //화면출력1, [반복]
{
	char order_datez8                     [  8];	char _order_datez8;                       //주문일자             
	char order_noz10                      [ 10];	char _order_noz10;                        //주문번호             
	char orgnl_order_noz10                [ 10];	char _orgnl_order_noz10;                  //원주문번호           
	...
	char anw_cld_mkt_orr_no1z10           [ 10];	char _anw_cld_mkt_orr_no1z10;             //신규자시장주문번호1  
	char anw_cld_mkt_orr_no2z10           [ 10];	char _anw_cld_mkt_orr_no2z10;             //신규자시장주문번호2  
	char sor_mkt_orr_noz10                [ 10];	char _sor_mkt_orr_noz10;                  //SOR시장주문번호      
	char rmt_mkt_cdz3                     [  3];	char _rmt_mkt_cdz3;                       //요청시장코드         
	char snd_mkt_cdz3                     [  3];	char _snd_mkt_cdz3;                       //전송시장코드         
	char sor_mkt_sli_ynz1                 [  1];	char _sor_mkt_sli_ynz1;                   //SOR시장분할여부      
	char sop_cnd_prz15                    [ 15];	char _sop_cnd_prz15;                      //정지조건가격         
	char rjt_qtyz18                       [ 18];	char _rjt_qtyz18;                         //거부수량             
	char rmt_mkt_nmz50                    [ 50];	char _rmt_mkt_nmz50;                      //요청시장명           
	char sop_cnd_pr_rch_ynz1              [  1];	char _sop_cnd_pr_rch_ynz1;                //정지조건가격도달여부 
	char can_qtyz18                       [ 18];	char _can_qtyz18;                         //취소수량             /*IOC자동취소*/
} Ts8180OutBlock1;

typedef struct tags8180OutBlock_IN    //Button정보
{
	char ctsz56                           [ 56];	char _ctsz56;                             //CTS                  
	char nextbutton                       [  1];	char _nextbutton;                         //NEXTBUTTON           
} Ts8180OutBlock_IN;

typedef struct tags8180
{
	Ts8180InBlock                     s8180inblock                          ;  //기본입력 
	Ts8180OutBlock                    s8180outblock                         ;  //화면출력 
	Ts8180OutBlock1                   s8180outblock1                   [ 20];  //화면출력1 , [반복]
	Ts8180OutBlock_IN                 s8180outblock_in                      ;  //Button정보 
} Ts8180;
...
{
	char act_noz11[11];	char _act_noz11;                          //계좌번호             
...
typedef struct tags8118InBlock    //기본입력
{
	char datez8[8];	char _datez8;                             //주문일자             
	char accnt_noz11[11];	char _accnt_noz11;                        //계좌번호             
	char order_noz10[10];	char _order_noz10;                        //주문번호             /*주문번호-앞에0을채워조회*/
	char trad_pswd_no_1z8[44];	char _trad_pswd_no_1z8;                   //거래비밀번호1        
	char trad_pswd_no_2z8[44];	char _trad_pswd_no_2z8;                   //거래비밀번호2        
} Ts8118InBlock;

typedef struct tags8118OutBlock1    //화면출력, [반복]
{
	char datez8[8];	char _datez8;                             //주문일자             
	char order_noz10[10];	char _order_noz10;                        //주문번호             
	char org_ord_noz10[10];	char _org_ord_noz10;                      //원주문번호           
	char issue_cdz6[6];	char _issue_cdz6;                         //종목코드             
	char issue_nmz20[20];	char _issue_nmz20;                        //종목명               
	char order_nmz10[10];	char _order_nmz10;                        //주문명               
	char slby_nmz10[10];	char _slby_nmz10;                         //매매명               
	char order_qtyz10[10];	char _order_qtyz10;                       //주문수량             
	char order_prcz9[9];	char _order_prcz9;                        //주문단가             
	char conc_qtyz10[10];	char _conc_qtyz10;                        //체결수량             
	char conc_amtz15[15];	char _conc_amtz15;                        //체결금액             
	char real_qtyz10[10];	char _real_qtyz10;                        //정취가능수량         
	char proc_nmz10[10];	char _proc_nmz10;                         //처리명               
	char reject_cdz4[4];	char _reject_cdz4;                        //거부코드             
	char comm_nmz10[10];	char _comm_nmz10;                         //매체명               
	char reject_qtyz10[10];	char _reject_qtyz10;                      //거부수량             
	char mo_cld_orr_dit_cdz8[8];	char _mo_cld_orr_dit_cdz8;                //모자주분구분유형     /*모자주문구분유형P주문/C-1주문/C-2주문*/
	char ord_cond_prcz11[11];	char _ord_cond_prcz11;                    //호가조건가격         /*호가조건가격-Stop가격*/
	char sor_gbz1[1];	char _sor_gbz1;                           //SOR구분              /*sor구분-S:SOR주문,0:일반주문*/
	char rmt_mkt_cdz3[3];	char _rmt_mkt_cdz3;                       //요청시장코드         /*요청시장코드-SOR,KRX,NXT*/
	char sli_ynz1[1];	char _sli_ynz1;                           //분할주문구분         /*분할주문구분-Y:SPLIT주문,N:일반주문*/
	char snd_mkt_cdz3[3];	char _snd_mkt_cdz3;                       //전송시장코드         /*전송시장코드-KRX,NXT,_:해당없음*/
	char k_reject_cdz4[4];	char _k_reject_cdz4;                      //거래소거부코드       /*거래소거부코드-(P)모주문번호에만사용*/
	char n_reject_cdz4[4];	char _n_reject_cdz4;                      //대체거래소거부코드   /*대체거래소거부코드-(P)모주문번호에만사용*/
	char can_qtyz10[10];	char _can_qtyz10;                         //취소수량             /*자동취소수량*/
} Ts8118OutBlock1;

typedef struct tags8118
{
	Ts8118InBlock                     s8118inblock;  //기본입력 
	Ts8118OutBlock1                   s8118outblock1[20];  //화면출력 , [반복]
} Ts8118;
...
typedef struct tagd2OutBlock    //출력
{
	...
	char ato_ord_tpe_chg[1];   //주문유형             
	char issue_nm[20];   //종목명               
	char rmt_mkt_cd[1];   //요청시장코드         
	char snd_mkt_cd[1];   //전송시장코드         
	char ord_cond_prc[11];   //호가조건가격         
	char sor_orrgb[1];   //sor주문구분          
	char stop_efforn_gb[1];   //스탑주문효력발생구분 
} Td2OutBlock;
...
typedef struct tagd3OutBlock    //출력
{
	...
	char snd_mkt_cd[1];   //전송시장코드         
	char ord_cond_prc[11];   //호가조건가격         
	char sor_orrgb[1];   //SOR구분              
	char mkt_order_qty[10];   //전송시장주문수량     
} Td3OutBlock;
